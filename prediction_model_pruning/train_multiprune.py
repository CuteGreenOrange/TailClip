...
def infer_test(model):
    ...
    torch.cuda.synchronize()
    end_time = time.time()
    print("only_infer_time:", (end_time-start_time)/10)  
...
def image_infer(model,img0_np,img1_np):
    ...
    img_pred = padder.unpad(img_pred)             

    img_pred_np = (img_pred[0].data.permute(1, 2, 0).cpu().numpy() * 255.0).astype(np.uint8) 
    return img_pred_np
...
if __name__ == "__main__":
    ...
    parser.add_argument('--op_mode', default='Infer', type=str, help='Train, Infer')
    ...
    parser.add_argument('--multipyramid', default=0, type=int)
    parser.add_argument('--percent_list',  nargs='+', type=float)
    ...
    args.model_name = "IFRNet_SPruned"
    if args.prune_mode =="PYRAMID_PRUNE":
        PYRAMID_PRUNE = True
        LAYER_PRUNE = False
        if args.multipyramid>0:
            MULTIPYRAMID = True
            PERCENT_LIST=args.percent_list
            print("prune_mode:{}, multiple, percent_list:{}".format(args.prune_mode,PERCENT_LIST))
        else:
    ...
    percent_filename=args.prune_percent
    output_csv_file = args.output_file
    if args.op_mode == "Infer":
        with torch.no_grad():

            model = Model(config=CONFIG).eval()
            if PYRAMID_PRUNE==True:
                if MULTIPYRAMID==True:
                    PYRAMID=1
                    PRUNE_CONFIGS={}
                    for percent in PERCENT_LIST:
                        PRUNE_CONFIGS={}
                        PRUNE_PERCENTAGE=percent
                    ...
                    PRUNE_CONFIGS = gen_pyramidconfig()
                    model = prune_network(model)
                model.load_state_dict(torch.load('./checkpoint/AMP04.pth'))
                img0_path="./img/img01.png"
                img1_path="./img/img03.png"
                imgpre_path=f"./img/imgPred.png"
                img0 = read(img0_path)
                img1 = read(img1_path)
                imgpre = image_infer(model,img0, img1 )
                imageio.imwrite(imgpre_path,imgpre)
            if LAYER_PRUNE == True:
