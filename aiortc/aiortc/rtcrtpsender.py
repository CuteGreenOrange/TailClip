...
class SkipFrameFraction:
    def __init__(self, existed_frames_no=1, predicted_frames_no=0) -> None:
        self.existed_no = existed_frames_no
        self.predicted_no = predicted_frames_no
    
    def group_no(self):
        return self.existed_no + self.predicted_no
...
class RTCRtpSender:
    ...
    def __init__(self, trackOrKind: Union[MediaStreamTrack, str], transport) -> None:
        ...
        # frame prediction
        self.frame_idx = -1  
        # self.skip_frame = SkipFrameFraction(1,1)  
        self.skip_frame = SkipFrameFraction(1,0)  
        self.run_rtp_frame_idx = 0

        # #GCC
        # self.gcc_Ar = 0
        # self.gcc_As = 0
        # self.__gcc_total_rtt = 0
    ...
    async def _handle_rtcp_packet(self, packet):
        if isinstance(packet, (RtcpRrPacket, RtcpSrPacket)):
            logger.info(f"[recv]\tRrSr\trecv_point:{time.time()}\tpkt:{packet}")
            for report in filter(lambda x: x.ssrc == self._ssrc, packet.reports):
                ...
                #     self.__gcc_total_rtt += rtt
                #     print("rtt(s): ",rtt, " total_rtt: ",self.__gcc_total_rtt)

          
    
                # loss_rate = round(report.fraction_lost/256,2)
                # if report.fraction_lost != 0:
                #     print("======================================report.fraction_lost is: {}, loss_rate is {}".format(report.fraction_lost, loss_rate))
                # if loss_rate > 0.1:
                #     self.gcc_As = min(self.gcc_Ar, int(self.gcc_As * (1-0.5*loss_rate)))
                # elif loss_rate < 0.02:
                #     self.As = min(self.gcc_Ar, int(self.gcc_As * 1.05))
            ...
            for seq in packet.lost:
                logger.info(f"[recv]\tRtcpPsfbPacket_NACK\trecv_point:{time.time()}\seq:{seq}\tpkt:{packet}")
                await self._retransmit(seq)
        elif isinstance(packet, RtcpPsfbPacket) and packet.fmt == RTCP_PSFB_PLI:    
            logger.info(f"[recv]\tRtcpPsfbPacket_PLI\trecv_point:{time.time()}\tpkt:{packet}")
            self._send_keyframe()
            ...
            try:
                ...
                if self._ssrc in ssrcs:
                    ...
                    # self.gcc_Ar = bitrate
                    # if self.gcc_As == 0:
                    #     self.gcc_As = self.gcc_Ar
                    # gcc_rate = min(self.gcc_Ar, self.gcc_As)
                    # # print("============================final rate: {}KBps, {}mbps,   Ar: {}KBps, {}mbps,   As: {}KBps, {}mbps"
                    # #       .format(rate/8000,rate/1000000,bitrate/8000,bitrate/1000000,self.As/8000,self.As/1000000))
                    # print("==================================final rate: {}mbps".format(gcc_rate / 1000000,))
                    # if self.__encoder and hasattr(self.__encoder, "target_bitrate"):
                    #     self.__encoder.target_bitrate = gcc_rate
                    if self.__encoder and hasattr(self.__encoder, "target_bitrate"):
                        self.__encoder.target_bitrate = bitrate
                        logger.info(f"[recv]\tRtcpPsfbPacket_APP\trecv_point:{time.time()}\tbitrate:{bitrate}\tpkt:{packet}")
    ...
    async def _next_encoded_frame(self, codec: RTCRtpCodecParameters):
        ...
        self.frame_idx += 1
        if self.skip_frame.predicted_no != 0:
            if self.frame_idx%(self.skip_frame.group_no()) not in range(self.skip_frame.existed_no):
                return 

        # # frame prediction
        # self.fp_frame_num += 1
        # if self.fp_frame_num % (self.fp_skip_frame+1) != 0:
        #     return
        ...
        if isinstance(data, Frame):
            ...
            payloads, timestamp = await self.__loop.run_in_executor(
                None, self.__encoder.encode, self.frame_idx, data, force_keyframe
            )
        else:
    ...
    async def _retransmit(self, sequence_number: int) -> None:
        ...
        if packet and packet.sequence_number == sequence_number:
            ...
            logger.info(f"[4-1][send][retransmit]\tpkt:{packet}")
    ...
    async def _run_rtp(self, codec: RTCRtpCodecParameters) -> None:
        ...
        try:
            while True:
                ...
                if enc_frame == None: continue

                # # frame prediction
                # if not fp_NotEncode:
                #     enc_frame = await self.__track.recv()
                # else:
                #     enc_frame = await self._next_encoded_frame(codec)
                #     if enc_frame == None:
                ...
                for i, payload in enumerate(enc_frame.payloads):
                    ...
                    packet_bytes = packet.serialize(self.__rtp_header_extensions_map)
                    logger.info(f"\t[4][send]\tframe_idx:{self.run_rtp_frame_idx}\tsend_point:{time.time():.3f}\tpkt:{packet}")
                    await self.transport._send_rtp(packet_bytes)
                    # logger.info(f"\t[4][send]\tframe_idx:{frame_idx}\tsend_point:{time.time():.3f}\tpkt:{packet}")
                ...
                # if frame_idx > 300*args.fps:
                # if self.run_rtp_frame_idx > 300*args.fps:
                if self.run_rtp_frame_idx > 300*args.fps:
                    self.__track.stop()
                    self.__track = None
                    logger.info(">>>>>>>>> FINISH 5MINS VIDEO")
                    self.__rtp_exited.set()
                # frame_idx += 1
                self.run_rtp_frame_idx += 1
    ...
    async def _run_rtcp(self) -> None:
        ...
        try:
            while True:
                ...
                logger.info(f"[send]\tSr\tsend_point:{time.time()}\tpkt:{packet}")
                ...
                if self.__cname is not None:
                    ...
                    logger.info(f"[send]\tBye\tsend_point:{time.time()}\tpkt:{packet}")
