...
MAX_BITRATE = 500000000  # 500 Mbps
...
class VpxPayloadDescriptor:
    ...
    def parse(cls: Type[DESCRIPTOR_T], data: bytes) -> Tuple[DESCRIPTOR_T, bytes]:
        ...
        if partition_start == 1 and partition_id == 0:
            # keyframe_indicator = not ((data[pos]>>7)&1)
            keyframe_indicator = not ((data[pos])&1)
            logger.debug(f">>>>>>>>>>>>>>> keyframe:{keyframe_indicator}")
        return obj, data[pos:]
...
def _vpx_assert(err: int) -> None:
    if err != lib.VPX_CODEC_OK:
        ...
        error_msg = "libvpx error: " + reason.decode("utf8")
...
class Vp8Decoder(Decoder):
    ...
    def decode(self, encoded_frame: JitterFrame) -> List[Frame]:
        ...
        logger.info(f"started_decode_point:{time.time()}")
        frames: List[Frame] = []
            ...
            while True:
                ...
                for p in range(3):
                    ...
                    for r in range(0, img.d_h // div):
                        ...
                        i_pos += i_stride
                        o_pos += o_stride
                logger.info(f"finished_decode_point:{time.time()}")
...
class Vp8Encoder(Encoder):
    def __init__(self) -> None:
        ...
        logger.critical("VP8ENCODER INIT")
    ...
    def encode(
        ...
        if not self.codec:
            ...
            logger.info(f"frame.width:{frame.width}\frame.height:{frame.height}")
            ...
            self.cfg.rc_buf_initial_sz = 50000
            self.cfg.rc_buf_optimal_sz = 60000
            self.cfg.rc_buf_sz = 100000
            ...
            #     self.codec, lib.VP8E_SET_CPUUSED, ffi.cast("int", -20)
            # )
            lib.vpx_codec_control_(
      
                # self.codec, lib.VP8E_SET_CPUUSED, ffi.cast("int", -6)
                self.codec, lib.VP8E_SET_CPUUSED, ffi.cast("int", -20)
                # self.codec, lib.VP8E_SET_CPUUSED, ffi.cast("int", -60)
            )
            lib.vpx_codec_control_(
                self.codec,
        ...
        while True:
            ...
            if not pkt:
                ...
                logger.critical(f"\t[3][vp8_enc]\tframe idx:{self.frame_idx}\tencode_time:{encode_time:.3f}\tencode_point:{time.time():.3f}\tsize:{pkt.data.frame.sz}\tkeyframe:{(pkt.data.frame.flags&1)==1}\trc_target_bitrate:{self.cfg.rc_target_bitrate}\theight:{self.cfg.g_h}")
                # logger.info(f">>>>>>>>>> encoded frame flags: {pkt.data.frame.flags}")
                # logger.info(f">>>>>>>>>> encoded frame flags (bin): {bin(pkt.data.frame.flags)}")
                # logger.info(f">>>>>>>>>> encoded frame pts: {pkt.data.frame.pts}")
            ...
            elif pkt.kind == lib.VPX_CODEC_PSNR_PKT:
                logger.info(f"total psnr: {pkt.data.psnr.psnr[0]}")
    ...
    def __update_config(self) -> None:
        ...
        logger.info("VP8ENCODE UPDATE BITRATE!!")
