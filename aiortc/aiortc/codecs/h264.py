...
import sys
sys.path.insert(0, "./frame_prediction")
...
DEFAULT_BITRATE = args.default_bitrate*1000  # 500 kbps
MIN_BITRATE = args.min_bitrate*1000  # 500 kbps
MAX_BITRATE = args.max_bitrate*1000  # 1 Mbps

MAX_FRAME_RATE = args.fps
PACKET_MAX = 1300
...
class H264Decoder(Decoder):
    def __init__(self) -> None:
        ...
        self.frame_idx = 0
        self.first_frame_point = None
        try:
            if args.out_yuv != '':
                self.output_yuv = open(args.out_yuv, 'wb')
        except Exception as ex:
            logger.error(f"RAWVIDEO ENCODER INIT FAIL:{ex}")

    def decode(self, encoded_frame: JitterFrame, first_pkt:RtpPacket) -> List[Frame]:
        try:
            time1 = time.time()
            packet = av.Packet(encoded_frame.data)
            packet.pts = encoded_frame.timestamp
            packet.time_base = VIDEO_TIME_BASE
            process_frame_time = time.time()-time1

            time1 = time.time()
            frames = self.codec.decode(packet)  # Decode a list of Frame from the given Packet
            decode_time = time.time()-time1

            time1 = time.time()
            if args.out_yuv!='':
                write_bytes = frames[0].to_ndarray().tobytes()
                self.output_yuv.write(write_bytes)
            write_time = time.time()-time1

            if self.first_frame_point == None:
                self.first_frame_point = time.time()
            relative_time = time.time()-self.first_frame_point

            logger.info(f"\t[3][libx264_dec]\tframe_idx:{self.frame_idx}\tfirst_pkt:{first_pkt}\tdecode_time_point:{time.time()}\trelative_time:{relative_time}\tdecode_time:{decode_time}\tprocess_frame_time:{process_frame_time}\twrite2file_time:{write_time}\theight:{frames[0].height}")

            if len(frames) > 1:
                
                logger.info(f"\t[3][vp8_decoder]\tfirst_pkt_seq:{first_pkt}\tonce_decode_frames_count:{len(frames)}")
...
class H264Encoder(Encoder):
    def __init__(self) -> None:
        ...
        self.frame_idx = 0
        logger.critical("H264ENCODER INIT")
    ...
    def _encode_frame(
        self, frame_idx:int, frame: av.VideoFrame, force_keyframe: bool
    ) -> Iterator[bytes]:
 
        # # note: change bitrate
        # if frame_idx >= 600:
        #     self.target_bitrate = 50000
        #     frame = frame.reformat(width=960, height=540)
        time1 = time.time()
        if self.codec and (
            ...
            logger.info("LIBX264 UPDATE BITRATE")
            self.buffer_data = b""
        ...
        if self.codec is None:
            try:
             ...
             宽
                    # bitrate=self.target_bitrate*0.8,
                )
        data_to_send = b""
        for package in self.codec.encode(frame):
            ...
            # logger.info(f"is keyframe:{package.is_keyframe}")

        # logger.info(f"codec bitrate:{self.codec.bit_rate}\ttarget bitrate:{self.target_bitrate}")
        # logger.info(f"size of data_to_send:{len(data_to_send)}")
        encode_time = time.time()-time1
        logger.info(f"\t[3-0][libx264_enc]\tframe idx:{frame_idx}\tsize:{len(data_to_send)}\tkeyframe:{package.is_keyframe}\ttarget_bitrate:{self.target_bitrate}\tcodec_bitrate:{self.codec.bit_rate}\tencode_time:{encode_time:.4f}\tencode_point:{time.time()}")
        if data_to_send:
            yield from self._split_bitstream(data_to_send)

    def encode(
        self, frame_idx:int, frame: Frame, force_keyframe: bool = False
        ...
        time1 = time.time()
        packages = self._encode_frame(frame_idx, frame, force_keyframe)
        encode_time = time.time()-time1

        timestamp = convert_timebase(frame.pts, frame.time_base, VIDEO_TIME_BASE)
        logger.info(f"\t[3-1][libx264_enc]\tframe idx:{frame_idx}\tencode_time:{encode_time:.3f}\tencode_point:{time.time():.3f}\theight:{frame.height}")
        self.frame_idx+=1
