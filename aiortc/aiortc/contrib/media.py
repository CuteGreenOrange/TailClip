...
def player_worker_decode(
    ...
    while not quit_event.is_set():
        ...
        if isinstance(frame, AudioFrame) and audio_track:
            ...
            frame_time = frame.time
            logger.info(f">>>[1][file]\tframe_idx:{frame_idx}\tdecode_time:{decode_time:.3f}\tput_queue_point:{time.time():.3f}")
            frame_idx+=1
    ...
    async def recv(self) -> Union[Frame, Packet]:
        ...
        while True:
            ...
            if isinstance(data, Frame):
                # data_time = data.time
                data_time = float(data.pts*self._player.stream_time_base)                
            ...
            if (
                ...
                if self._start is None:
                    ...
                    sample_relative = time.time()-self.start_point
                    logger.info(f"\t[2][sample]\tframe_idx:{self.frame_idx}\twait_time:{wait:.3f}\tsample_point:{time.time():.3f}\tdata_time:{data_time:.3f}\tsample_relative:{sample_relative:.4f}")
                else:
            ...
            if self.frame_idx > 300*args.fps:
                self.stop()
            return data
        ...
        for stream in self.__container.streams:
            if stream.type == "audio" and not self.__audio:
                ...
                if self.__decode:
                    ...
                    stream.thread_count = 16
                    logger.debug("set stream thread_count")
