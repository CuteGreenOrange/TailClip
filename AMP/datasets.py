...
class Train_Dataset(Dataset):
    def __init__(self, dataset_dir='/home/data', augment=True):
        self.dataset_dir = dataset_dir
        self.image_paths = sorted(os.listdir(os.path.join(self.dataset_dir, "seqs")))
        ...
        for i in range(len(self.image_paths)-2):
            self.img0_list.append(os.path.join(self.dataset_dir, "seqs", self.image_paths[i]))
            self.imgt_list.append(os.path.join(self.dataset_dir, "seqs", self.image_paths[i+1]))
            self.img1_list.append(os.path.join(self.dataset_dir, "seqs", self.image_paths[i+2]))
            self.flow_t0_list.append(os.path.join(dataset_dir, "flow", self.image_paths[i+1].split(".")[0]+"_t0.flo"))
            self.flow_t1_list.append(os.path.join(dataset_dir, "flow", self.image_paths[i+1].split(".")[0]+"_t1.flo"))
...
class Test_Dataset(Dataset):
    def __init__(self, dataset_dir='/home/data'):
        self.dataset_dir = dataset_dir
        self.image_paths = sorted(os.listdir(os.path.join(self.dataset_dir, "seqs")))
        self.img0_list = []
        self.imgt_list = []
    ...
    def __getitem__(self, idx):
        ...
        img0 = torch.from_numpy(img0.transpose((2, 0, 1)).astype(np.float32) / 255.0).unsqueeze(0)
        imgt = torch.from_numpy(imgt.transpose((2, 0, 1)).astype(np.float32) / 255.0).unsqueeze(0)
        img1 = torch.from_numpy(img1.transpose((2, 0, 1)).astype(np.float32) / 255.0).unsqueeze(0)
        flow = torch.from_numpy(flow.transpose((2, 0, 1)).astype(np.float32)).unsqueeze(0)

        img_padder = InputPadder(img0.shape, 16)
        img0, imgt, img1 = img_padder.pad(img0, imgt, img1)
        flow_padder = InputPadder(flow.shape,16)
        flow = flow_padder.pad(flow)[0]
...
class Pre_Train_Dataset(Dataset):
    def __init__(self, dataset_dir='/home/data', augment=True):
        ...
        self.flow_t0_list = []
        self.flow_t1_list = []
        for i in range(len(self.image_paths)-3):
            self.img0_list.append(os.path.join(self.dataset_dir, "seqs", self.image_paths[i]))
            self.imgt_list.append(os.path.join(self.dataset_dir, "seqs", self.image_paths[i+3]))
            self.img1_list.append(os.path.join(self.dataset_dir, "seqs", self.image_paths[i+2]))
            self.flow_t0_list.append(os.path.join(dataset_dir, "flow", self.image_paths[i+3].split(".")[0]+"_t0.flo"))
            self.flow_t1_list.append(os.path.join(dataset_dir, "flow", self.image_paths[i+3].split(".")[0]+"_t1.flo"))
...
class Pre_Test_Dataset(Dataset):
    def __init__(self, dataset_dir='/home/data'):
        self.dataset_dir = dataset_dir
        self.image_paths = sorted(os.listdir(os.path.join(self.dataset_dir, "seqs")))
        self.img0_list = []
